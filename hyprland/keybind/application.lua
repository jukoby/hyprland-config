...
local apps = require("hyprland.namespace.applications")

local fp = "flatpak run "

hl.bind("SUPER + SUPER_L",  uwsm.dsp("nc -U " .. os.getenv("XDG_RUNTIME_DIR") .. "/walker/walker.sock"), { desc = "Walker" })

hl.bind("F19",              uwsm.dsp(apps.browser),                                  { desc = "apps.browser" })
...
hl.bind("ALT + F19",        uwsm.dsp(apps.browser .. " https://github.com"),         { desc = "Github" })

hl.bind("F20",              uwsm.dsp("vscodium.desktop"),                       { desc = "VSCode" })
hl.bind("SHIFT + F20",      uwsm.dsp(apps.file_manager),                             { desc = "File Manager" })
hl.bind("SUPER + F20",      uwsm.dsp(fp .. "com.github.xournalpp.xournalpp"),   { desc = "Xournal++" })
-- hl.bind("CTRL + F20")
-- hl.bind("ALT + F20")

hl.bind("F21",              uwsm.dsp(apps.terminal),                 { desc = "apps.terminal" })
hl.bind("SHIFT + F21",      uwsm.dsp(""),                       { desc = "SSH" })     -- TODO
hl.bind("SUPER + F21",      uwsm.dsp(apps.terminal .. " -e yay"),    { desc = "yay" })
